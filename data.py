    ...
    if element.tag == "node" or element.tag == "way" :
        ...
        if tag_keys != []:
            if any([ xx.attrib["k"][:4] == "addr" for xx in tag_keys ]):
                   node["address"] = {}
            for x in tag_keys:
                # print x.attrib['k'], "with value", x.attrib['v']
                ...
                if y[:4] == "addr":
                    ...
                    if len(yy) <= 2 and (re.match(problemchars,yy[1]) == None):
                        node["address"][yy[1]] = z
                        #print node
                elif (re.match(lower_colon, y) != None):
                    node[y] = z
        for x in kys:
            if x in CREATED:
                # print x, element.attrib[x]
                node["created"][x] = element.attrib[x]
            elif (x != "lon") and (x != "lat"):
                node[x] = element.attrib[x]
        if ("address" in node.keys()) and( node["address"] != {}):
            pprint.pprint(node)
        return node
    else:
        return None
